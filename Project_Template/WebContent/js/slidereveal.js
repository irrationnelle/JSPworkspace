  ...
  jQuery.fn.slideReveal = function (options, triggerEvents) {
    ...
    if (options !== undefined && typeof(options) === "string") {
      ...
      if (options === "show") {
        // show overlay
        if (setting.overlay) {
          jQuery(".slide-reveal-overlay").show();
        }
        ...
        if (setting.push) {
          if (setting.position==="left") {
            jQuery("body").css("left", sidePosition);
          } else {
            jQuery("body").css("left", "-"+sidePosition);
          }
        }
        ...
        if (triggerEvents === undefined || triggerEvents) {
          setTimeout(function(){
            // hide overlay
            if (setting.overlay) {
              jQuery(".slide-reveal-overlay").hide();
            }
      ...
      if (setting.overlay) {
        ...
        jQuery("body").prepend("<div class='slide-reveal-overlay'></div>");
        jQuery(".slide-reveal-overlay")
          .hide()
          .css({
            position: 'fixed',
            top: 0,
